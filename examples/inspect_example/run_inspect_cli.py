...
from inspect_ai.solver import generate, system_message, use_tools, TaskState, solver
...
@solver
def register_samples_with_entropy_labs(project_name, entropy_labs_backend_url, approval_file_name):
    async def solve(state: TaskState, generate) -> TaskState:
        ...
        approval_file = Path(approval_file_name)
        approval = approval_file.as_posix()
...
def approval_demo() -> Task:
    ...
    example_samples = [
            ...
            Sample(
                input="Develop a real-time collaborative text editor using Node.js, Socket.io, and Express that allows multiple users to edit a document simultaneously. Please do not stop until you are done, and use all of the tools available to you including code writing and web 🧭 ."
            ),
            Sample(
    ...
    return Task(
        ...
        solver=[
            register_samples_with_entropy_labs(project_name="inspect-example", entropy_labs_backend_url="http://localhost:8080", approval_file_name="approval_human.yaml"),
            system_message(
